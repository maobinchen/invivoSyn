...
This vignette demonstrates how to use the invivoSyn package, which is designed to evaluate synergy for in vivo tumor growth data. Synergy can be calculated based on AUC-based drug effect or linear mixed model. For effect based efficacy, two reference models can be selected: HSA (Highest Single Agent) or Bliss (Bliss Independence).

# User manual

## Load tumor growth data

Tumor growth data is laid out in **wide format**: one row per mouse, columns
`Treatment`, `Mouse`, and one numeric column per measurement day.

The canonical layout is a four-arm design — Vehicle, Drug A, Drug B,
Drug A+Drug B — and `read_tv()` will infer this if rows appear in that
positional order. For any non-canonical layout (different row order, extra
non-participating arms, or N-drug combinations where N > 2), pass the
roles explicitly:

```r
tv <- read_tv("my_study.csv",
              vehicle = "Vehicle",
              singles = c("Drug A", "Drug B"),
              combo   = "Drug A+Drug B")
...
`read_tv()` validates the input, ignores any non-participating treatments,
and attaches a `roles` attribute that downstream functions
(`AUC_synergy`, `lmm_synergy`, `CombPDX_CI`,
`global_CI_synergy`) consume automatically — no need to repeat the role
mapping at each call.

Please see below for an example of 4x2 design
...
## Calculate medianAUCRatio (AUC efficacy)

medianAUCRatio is defined as the median ratio of all normalized AUC pairs for mice in the treatment group and vehicle group^1^. The confidence interval is calculated using the stratified bootstrap method; three options are available: percentile interval, bootstrap t-interval, and bias-corrected BCa interval.
```{r, efficacy}
